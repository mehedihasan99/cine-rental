...
export default function ShoppingDetails({ onClose }) {
  const { state, dispatch } = useContext(MovieContext)
  const handleCardRemove = (e, item) => {
    e.stopPropagation()
    dispatch({
      type: 'REMOVE_TO_CARD',
      payload: item,
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[420px] sm:max-w-[600px] lg:max-w-[790px] p-4 max-h-[90vh] overflow-auto">
        <div className="bg-white shadow-md dark:bg-[#12141D] rounded-2xl overflow-hidden p-5 md:p-9">
          ...
          {state.shoppingCardData.length > 0 ? (
            state.shoppingCardData.map((item) => (
              <div
                key={item.id}
              ...
              >
                <div className="grid grid-cols-[1fr_auto] gap-4">
                  ...
                  <div className="flex justify-between gap-4 items-center">
                    <button
                      onClick={(e) => handleCardRemove(e, item)}
            ...
            <p className="text-2xl text-green-400">Please add some movies</p>
          )}
